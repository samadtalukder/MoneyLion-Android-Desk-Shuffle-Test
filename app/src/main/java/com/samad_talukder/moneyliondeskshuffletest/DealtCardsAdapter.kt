...
open class DealtCardsAdapter(context: Context, drawables: List<Int>) :
    ArrayAdapter<Int>(context, 0, drawables) {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        listItemView ?: run {
            listItemView =
                LayoutInflater.from(context).inflate(R.layout.selected_card_item, parent, false)
        }
        ...
        image?.let {
            if (selectedPosition == position) {
                it.setColorFilter(R.color.black, PorterDuff.Mode.SRC_ATOP)
            } else {
                it.colorFilter = null
            }
        }
    ...
    fun updateDealtPosition(position: Int?) {
