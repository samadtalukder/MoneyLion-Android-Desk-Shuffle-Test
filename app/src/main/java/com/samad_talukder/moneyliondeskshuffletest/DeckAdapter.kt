...
import com.samad_talukder.moneyliondeskshuffletest.databinding.DeckItemBinding

class DeckAdapter(private val itemTapListener: (Int) -> Unit) :
    ListAdapter<Int, DeckAdapter.DeckViewHolder>(GenericDiffUtil<Int>()) {

    fun updateDeck(drawables: List<Int>) {
        val newList = ArrayList(drawables)
        submitList(newList)
